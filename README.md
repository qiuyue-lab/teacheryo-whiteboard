...
面向老师的课堂互动套件：随机点名、便签墙、选择题、填空题、词云、材料学习。
...
- 想让 AI 帮你**使用**它 → 看 **[AI-PROMPT.md](AI-PROMPT.md)**，里面有几段写好、可以直接复制给 AI 的话，
  覆盖「跑起来 / 部署 / 接云端 / 改功能 / 报 bug」五种情况
- 想让 AI 帮你**修改**它 → AI 打开项目会自动读 **[AGENTS.md](AGENTS.md)**
  （`CLAUDE.md` 也已指向它），里面是改这个项目必须遵守的约定和验证方法
...
2. **加互动**：进课程 → 添加互动，选随机点名 / 便签墙 / 选择题 / 填空题 / 词云 / 材料学习
3. **调顺序**：按住每行右侧的 `⠿` 手柄上下拖动，课堂流程就按新顺序排好
4. **换封面**：鼠标移到课程卡片上，点「🖼 换封面」上传自己的图（想还原就点「↩︎ 恢复默认」）
5. **下发**：进互动页 → 点「下发二维码」，学生扫码或点链接进
6. **看结果**：学生提交的内容实时汇聚到大屏，可点赞、可拖动、可整理
7. **导出**：导出成文本存档
...
- 点「👍 按点赞排序」，白板会把便签按点赞数**从多到少**铺成一张整齐的网格
  （排完自动对准视野，不用自己缩放找）
